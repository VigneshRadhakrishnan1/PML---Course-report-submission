...
#summary(training)
```
It was observed that there were multiple NAs in a lot of the variables. We will be removing those variables from our data set and create a usable tidy dataset. We are left with 60 variables after the cleanu. Further variables that cannot be used in modeling - userid, time etc.- are removed.
```{r}
...
We will be splitting the training datset 70-30 using caret. The intent is to try multiple classification model and compare for best fit. We will be using - randomForest, c&rt, c5.0. Please make sure that the following packages are installed already in your system else the models won't work.
```{r}
#Loading packages
library(caret)
library(rpart)
...
```
The first model was developed for a Random Forest algorithm using randomForest package. The model has an accuracy of 99.26%
```{r}
...
Control2<-trainControl(method="cv", number=6, allowParallel=T, verbose=T)
fit1 <- train(classe ~ ., data=train1, method="rf", trControl=Control2, verbose=F)
...
predFin<-predict(fit1, newdata=testData) # for prediction dataset
```

Other models were also attempted using the caret package. The knn model had a much lower accuracy at 90.62%. The prediction on the prediction data set for the 20 cases though were accurate 
```{r}
...
predFin2<-predict(fit2, newdata=testData) # for prediction dataset
```

A Neural network model was also tried. 
```{r}
fit3 <- train(classe ~ ., data=train1, method="nnet", preProcess="range", tuneLength=2, trace=FALSE, maxit=100)
...
predFin2<-predict(fit3, newdata=testData) # for prediction dataset
```
Variable importance for the model was looked to relative importance of the factors in the model
```{r}
varImp(fit1)
```
As suggested in the course the below code was used to create individual text files for each predicted case, commented out here. The files will get created in the working directory. The results were tested on the submission page and had 100% accuracy.
```{r}
getwd()
